...
/* 
  Your task is to use your knowledge of accessing property values and array indices 
  to output the commit message for the push event with an id of 8030403992.
*/
// HARD-CODED ANSWER by finding exact id in array list [index: 6], commit [index: 0]
console.log(githubData[6].payload.commits[0].message);

// DYNAMIC ANSWER using function
function printGithubCommitMessages(dataId){
  githubData.forEach(function(element){
    // console.log the commit message if the id matches
    if (element.id === dataId) {
      let commitList = element.payload.commits
...
printGithubCommitMessages('8030403992')